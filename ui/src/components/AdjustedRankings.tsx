...
import type { Teams } from "@ui/utilities/getPageData";

type AdjustedRankingsProps = { teams: Teams };
  ...
  <div className="flex flex-col gap-8">
    ...
    <div className="inline-block min-w-full align-middle">
      <table className="min-w-full divide-y divide-gray-300">
        ...
        <tbody className="xs:tracking-normal divide-y divide-gray-200 bg-white tracking-tight">
          {[...teams]
            .sort((a, b) => a.adjustedRanking - b.adjustedRanking)
            .map((team) => (
              <tr key={team.teamName} className="xs:text-sm whitespace-nowrap text-xs">
                <td className="py-2 pl-2 pr-1 font-medium text-gray-900">{team.adjustedRanking}</td>
                <td className="px-2 py-2 font-medium text-gray-900 sm:px-3">
                  {team.shortTeamName}
                </td>
                <td
                  className={clsx(
                    "w-14 px-1 py-2 text-gray-500 sm:px-3",
                ...
                <td className="px-2 py-2 text-center tabular-nums text-gray-500 sm:px-3">
                  {team.adjusted.wins.toFixed(1)} - {team.adjusted.losses.toFixed(1)}
                </td>
                <td className="px-2 py-2 text-center tabular-nums text-gray-500 sm:px-3">
